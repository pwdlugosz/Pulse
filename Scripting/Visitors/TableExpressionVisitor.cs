...
namespace Pulse.Scripting
{
    ...
    //        foreach (PulseParser.T_join_onContext ctx in context.t_join_on_set().t_join_on())
    //        {
    ...
    //        ScalarExpressionSet sec = RecordExpressionVisitor.Render(sev, context.record_expression());
    ...
    //        ScalarExpressionSet grouper = RecordExpressionVisitor.Render(G_Visitor, context.t_fold().record_expression());
    ...
    //        // Get the selector //
    //        ScalarExpressionSet selector = new ScalarExpressionSet(Schema.Join(grouper.Columns, folds.Columns), 0);
    //        if (context.record_expression() != null)
    //            selector = RecordExpressionVisitor.Render(S_Visitor, context.record_expression());
    ...
    //        ScalarExpressionSet select = RecordExpressionVisitor.Render(vis, context.record_expression());
    ...
    //        // Build a visitor //
    //        ScalarExpressionVisitor vis = this.SeedVisitor.CloneOfMe();
    //        RecordExpressionVisitor rvis = new RecordExpressionVisitor(vis);

    //        // Get the first item //
    //        ScalarExpressionSet fields = rvis.Render(context.record_chain().record_expression()[0]);
    ...
    //            for (int i = 1; i < context.record_chain().record_expression().Length; i++)
    //            {
    //                int ExpectedLen = fields.Count;
    //                fields = rvis.Render(context.record_chain().record_expression()[i]);
