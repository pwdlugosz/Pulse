    ...
    public sealed class ScalarExpressionOptimizer
    {
        ...
        private ScalarExpression CompactCancleOut(ScalarExpression Node)
        {
            ...
            if (name == ScalarExpressionFunction.NAME_SUB)
                return new ScalarExpressionConstant(Node.ParentNode, Cell.ZeroValue(CellAffinity.DOUBLE));

            // Check for A / A -> 0 //
            if (name == ScalarExpressionFunction.NAME_DIV || name == ScalarExpressionFunction.NAME_CDIV)
                return new ScalarExpressionConstant(Node.ParentNode, Cell.OneValue(CellAffinity.DOUBLE));
        ...
        private ScalarExpression CompactMultDivMod(ScalarExpression Node)
        {
            ...
            {
                this._Tocks++;
                return new ScalarExpressionConstant(Node.ParentNode, Cell.NULL_DOUBLE);
            }
        ...
        public static bool IsStaticZero(ScalarExpression Node)
        {
            if (Node.Affinity == ScalarExpressionAffinity.Value)
                return (Node as ScalarExpressionConstant).Value == Cell.ZeroValue(Node.ExpressionReturnAffinity());
            return false;
        }
        ...
        {
            if (Node.Affinity == ScalarExpressionAffinity.Value)
                return (Node as ScalarExpressionConstant).Value == Cell.OneValue(Node.ExpressionReturnAffinity());
            return false;
        }
        ...
        public static ScalarExpressionCollection CompactTree(Host Host, ScalarExpressionCollection Tree)
        {
